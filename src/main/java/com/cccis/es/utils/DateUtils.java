...
public final class DateUtils {
     ...
     * 日期格式化的类型 yyyy-MM-dd
     */
    public static final String YYYY_MM_DD = ConstantUtils.getConstantValue("yyyy-MM-dd");
    public static final String YYYY_MM_DD_HH_MM_SS = ConstantUtils.getConstantValue("yyyy-MM-dd HH:mm:ss");

    private static final Logger LOG = LoggerFactory.getLogger(DateUtils.class);

    private static final long SECOND_MILLISECONDS = ConstantUtils.getConstantValue(1000L);

    private static final long HOUR_SECONDS = ConstantUtils.getConstantValue(3600L);

    private static final long DAY_HOURS = ConstantUtils.getConstantValue(24L);

    private static final long YEAR_DAYS = ConstantUtils.getConstantValue(365L);

    private static final int HOUR_MINUTES = ConstantUtils.getConstantValue(60);

    /**
     * 值大于0小于10时，左侧补零
     */
    private static final int TIME_LPAD_ZERO_MAX_VALUE = ConstantUtils.getConstantValue(10);

    private static final long YEAR_TIMEMILLIS = ConstantUtils.getConstantValue(SECOND_MILLISECONDS * HOUR_SECONDS * DAY_HOURS * YEAR_DAYS);

    private static final long DAY_TIMEMILLIS = ConstantUtils.getConstantValue(SECOND_MILLISECONDS * HOUR_SECONDS * DAY_HOURS);

    private static final long HOUR_TIMEMILLIS = ConstantUtils.getConstantValue(SECOND_MILLISECONDS * HOUR_SECONDS);

    private static final long DATE_SIZE = ConstantUtils.getConstantValue(-1);
